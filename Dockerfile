...
WORKDIR /opt/polaris-test-platform
COPY /web ./web
RUN cd /opt/polaris-test-platform/web && npm i --registry=https://registry.npmmirror.com && npm run build
...
COPY --from=web /opt/polaris-test-platform/web/dist /opt/polaris-test-platform/web/dist
